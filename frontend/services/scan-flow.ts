...
/**
 * Scan-first flow: the backend auto-creates the plant if no plantId is given,
 * and updates health/lastScannedAt internally. Synchronization now only
 * refreshes the local Plant object for UI consistency.
 */
export function createScanFlow(
  client: Pick<ApiClient, "scanPlant" | "updatePlantHealth" | "fetchPlant"> = api,
) {
  ...
  const publish = (next: Partial<ScanFlowState>) => {
    state = { ...state, ...next };
  ...
  return {
    ...
    async scan(request: ScanRequest): Promise<ScanResponse> {
      if (busy)
        throw new ApiError(
          "busy",
          "A scan or synchronization is already running",
        );
      if (state.report && state.phase !== "complete")
        throw new ApiError(
          "busy",
          "Synchronize or reset the retained report before another scan",
        );
      busy = true;
      const token = ++generation;
      controller = new AbortController();
      const signal = controller.signal;
      // plantId comes from request OR will be set from scan response
      ...
      try {
        ...
        if (token === generation && !state.report)
          publish({ phase: "error", error: asApiError(error) });
        throw error;
      } finally {
        if (token === generation) {
          busy = false;
    ...
    async retrySynchronization(): Promise<void> {
      if (busy)
        throw new ApiError("busy", "Synchronization is already running");
      if (!state.report)
        throw new ApiError(
          "validation",
          "No successful report to synchronize",
        );
      ...
      controller = new AbortController();
      try {
        await synchronize(token, controller.signal);
      } finally {
        if (token === generation) {
          busy = false;
          controller = undefined;
    ...
    cancel() {
      ...
      publish(
        state.report
          ? { phase: "error", synchronizationError: error }
          : { phase: "error", error },
      );
    },
    reset() {
