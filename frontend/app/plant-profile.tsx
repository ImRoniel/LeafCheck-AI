...
export default function PlantProfile() {
  ...
  useFocusEffect(useCallback(() => { void attempt; const controller = new AbortController(); setPlant(null); setLoading(true); setError(null); if (!id) { setError('Missing plant ID'); setLoading(false); return; } fetchPlant(id, { signal: controller.signal }).then(p => { if (!controller.signal.aborted) setPlant(p); }).catch(e => { if (!controller.signal.aborted) setError(e.message); }).finally(() => { if (!controller.signal.aborted) setLoading(false); }); return () => controller.abort(); }, [id, attempt]));
